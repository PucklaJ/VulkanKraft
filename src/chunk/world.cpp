...
void World::check_aabb(physics::AABB &aabb) const {
  // Get the chunks in which the AABB resides
  const auto chunk_pos(get_chunk_position(aabb.position));
  if (m_chunks.find(chunk_pos) == m_chunks.end()) {
    // There is no chunk at the position of the AABB
    return;
  }

  std::vector<std::shared_ptr<Chunk>> colliding_chunks;
  colliding_chunks.reserve(9);

  // Get the chunk and all its eight neighbors
  const auto chunk(m_chunks.at(chunk_pos));
  if (auto chunk_aabb(chunk->to_physics_aabb()); chunk_aabb.collide(aabb)) {
    colliding_chunks.push_back(chunk);
  }

  if (auto left(chunk->get_left()); left) {
    if (auto chunk_aabb(left->to_physics_aabb()); chunk_aabb.collide(aabb)) {
      colliding_chunks.push_back(left);
    }
  }
  if (auto right(chunk->get_right()); right) {
    if (auto chunk_aabb(right->to_physics_aabb()); chunk_aabb.collide(aabb)) {
      colliding_chunks.push_back(right);
    }
  }
  if (auto front(chunk->get_front()); front) {
    if (auto chunk_aabb(front->to_physics_aabb()); chunk_aabb.collide(aabb)) {
  ...
  if (auto back(chunk->get_back()); back) {
    if (auto chunk_aabb(back->to_physics_aabb()); chunk_aabb.collide(aabb)) {
...
#endif
              aabb.position[index] += push[index];
            }
          }
        }
      }
