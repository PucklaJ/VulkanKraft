    ...
    for (size_t x = 0; x < block_width; x++) {
      for (size_t y = 0; y < block_height; y++) {
        for (size_t z = 0; z < block_depth; z++) {
          ...
          _check_faces_of_block(this, x, y, z, block.front_face(),
                                block.back_face(), block.right_face(),
                                block.left_face(), block.top_face(),
                                block.bot_face());
        }
      }
    }
    ...
    for (int z = -1; z <= 1; z++) {
      for (int y = -1; y <= 1; y++) {
        ...
        if ((pos.x >= 0 && pos.y >= 0 && pos.z >= 0) &&
          ...
          _check_faces_of_block(this, pos.x, pos.y, pos.z, block.front_face(),
                                block.back_face(), block.right_face(),
                                block.left_face(), block.top_face(),
                                block.bot_face());
        }
...
void Chunk::_check_faces_of_block(const Chunk *chunk, const size_t x,
